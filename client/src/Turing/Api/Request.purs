...
-- | functions that our production monad will leverage in `Turing.AppM` to implement our various
-- | app capabilities.
module Turing.Api.Request
...
import Turing.Api.Endpoint (Endpoint(..), endpointCodec)
import Turing.Data.Email (Email)
import Turing.Data.Email as Email
...
-- | Turing uses a REST API and secures certain endpoints using a JSON Web Token (JWT). That means
...
-- |
-- | Check out the `Turing.Capability.Resource.*` modules for examples of the high-level functions
...
-- | request functions defined below and in the `Turing.Api.Utils` modules.
...
-- | particular key. They'll be used as part of our production monad, `Turing.AppM`.
