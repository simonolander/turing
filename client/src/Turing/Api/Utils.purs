...
-- | monad, `Turing.AppM`.
module Turing.Api.Utils where

import Prelude

import Affjax (request)
import Turing.Api.Request (BaseURL, RequestOptions, Token, defaultRequest, readToken, writeToken)
import Turing.Capability.LogMessages (class LogMessages, logError)
...
-- | `baseUrl` field from the app environment. See `Turing.AppM` for examples of this in action.
mkRequest
  :: forall m r
   . MonadAff m
...
-- | the asynchronous request. See `Turing.AppM` for examples of this in action.
mkAuthRequest
  :: forall m r
   . MonadAff m
  => MonadAsk { baseUrl :: BaseURL | r } m
...
-- | dealing with malformed responses. See `Turing.AppM` for examples of this in practice.
