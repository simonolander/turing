...
-- | the application monad, `Turing.AppM`.
-- |
...
module Turing.Capability.LogMessages where

import Prelude

import Turing.Capability.Now (class Now)
import Turing.Data.Log (Log, LogReason(..), mkLog)
...
-- | `Log` type from `Turing.Data.Log`. This type can only be constructed using helper functions
