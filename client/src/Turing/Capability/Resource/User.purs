...
-- | any application code besides the application monad, `Turing.AppM`. In addition, we can test
...
module Turing.Capability.Resource.User where

import Prelude

import Turing.Api.Request (LoginFields, RegisterFields)
import Turing.Data.Email (Email)
import Turing.Data.Profile (Profile, ProfileRep, ProfileWithEmail, Author)
...
-- | `Turing.Data.Profile` module for more details.
...
class Monad m <= ManageUser2 m where
    getCurrentUser2 :: m (Maybe UserCredential)
