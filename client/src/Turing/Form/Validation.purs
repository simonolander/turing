...
module Turing.Form.Validation where
...
-- | `e`, which represents the possible error type which can result from the validator. For Turing
...
-- | This validator ensures that an input string is a valid username. Usernames in Turing use the
-- | smart constructor pattern, so we can't construct a username directly -- we'll need to defer
-- | to the `parse` helper function exported by `Turing.Data.Username`. Since that function returns
